...
class GenerateLocalizedCommand extends Command
{
    ...
    {
        $devNext = WayfinderVariant::isDevNext();

        $skipActions = (bool) $this->option('skip-actions') || $devNext;
        $skipRoutes = (bool) $this->option('skip-routes') || $devNext;

        if ($devNext && ! ($this->option('skip-actions') && $this->option('skip-routes'))) {
            $this->components->info(
                'laravel/wayfinder dev-next detected — actions and routes are generated by Wayfinder itself; generating translations only.',
            );
        }
        ...
        // The stable helper owns getLocale()/resolveLocalizedUrl(). Under
        // dev-next Wayfinder generates wayfinder/index.ts itself, so writing
        // ours would clobber every generated route in the project.
        if (! $devNext) {
            $this->writeWayfinderHelperFile();
        }

        $this->writeLocalesFile($devNext);

        $routes = $skipActions && $skipRoutes ? collect() : $this->wrapRoutes();
        ...
        if (! $this->option('skip-translations')) {
            $this->warnOnUnlistedDefaultLocale();

            $path = $this->writeTranslations($devNext);
    ...
    /**
     * Wrap the application's routes in the Wayfinder route wrapper.
     *
     * Only called when actions or routes are actually being generated: the
     * wrapper extends `Laravel\Wayfinder\Route`, which the dev-next branch
     * does not ship, so merely constructing it there is a fatal error.
     *
     * @return Collection<int, Route>
     */
    private function wrapRoutes(): Collection
    {
        ...
        return collect($this->router->getRoutes())->map(function (BaseRoute $route) use ($globalUrlDefaults) {
    ...
    /**
     * The default locale's catalog seeds the whole TranslationKey union, so a
     * default that is not in `wayfinder-i18n.locales` yields an empty union
     * and every t() call silently falls through to its own key.
     */
    private function warnOnUnlistedDefaultLocale(): void
    {
     ...
     * union.
     *
     * The two lines keep their default locale in different config files: the
     * stable line in `wayfinder-i18n.default`, dev-next in
     * `wayfinder-locales.default_locale` (which its route generation already
     * reads). Making a dev-next app declare the same locale twice — and
     * silently emitting the wrong catalog when the two disagree — is a trap,
     * so under dev-next the route-side value wins when it is set. It ships as
     * null, so apps that never set it keep falling back to `wayfinder-i18n`.
     */
    private function defaultLocale(): string
    {
        if (WayfinderVariant::isDevNext()) {
            $routeDefault = config('wayfinder-locales.default_locale');

            if (is_string($routeDefault) && $routeDefault !== '') {
                return $routeDefault;
            }
        }
    ...
    /**
     * @param  bool  $withLocaleStore  Emit the active-locale accessors here too.
     *                                 On the stable line they live in the
     *                                 package's own wayfinder/index.ts; under
     *                                 dev-next that file belongs to Wayfinder,
     *                                 so this module has to stand alone.
     */
    private function writeLocalesFile(bool $withLocaleStore = false): void
    {
        $previousPathDirectory = $this->pathDirectory;
        $this->pathDirectory = 'wayfinder';
    ...
    private function writeTranslations(bool $localesModuleOwnsLocaleStore = false): string
    {
        $previousPathDirectory = $this->pathDirectory;
        ...
        (new TranslationWriter($this->files))->write(
            $base,
            $collected,
            $this->locales(),
            $this->defaultLocale(),
            $localesModuleOwnsLocaleStore,
        );
