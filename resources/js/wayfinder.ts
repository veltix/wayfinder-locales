...
/**
 * The active locale, used to resolve localized route URLs. Mirrors the shape
 * of `setUrlDefaults` — pass a value or a thunk (e.g. `() => page.props.locale`).
 */
let currentLocale: () => string | null = () => null;

export const setLocale = (locale: Locale | (() => Locale | null)) => {
    currentLocale = typeof locale === "function" ? locale : () => locale;
};

export const getLocale = (): string | null => currentLocale();

/**
 * Select the URL for the active locale from a localized route definition,
 * falling back to the route's default locale.
 */
export const resolveLocalizedUrl = (
    urls: Record<string, string>,
    defaultLocale: string,
): string => urls[currentLocale() ?? defaultLocale] ?? urls[defaultLocale];

export type LocalizedRouteDefinition<TMethod extends Method | Method[]> = {
